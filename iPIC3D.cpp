...
int main (int argc, char **argv) {
    ...
    if (myrank==0){
        mpi->Print(); vct->Print();  col->Print(); col->save();
    }
    ...
    double *Ke = new double[ns]; double *momentum = new double[ns];
    ...
    for (int cycle = first_cycle; cycle < (col->getNcycles()+first_cycle); cycle++){
        ...
        if (cycle%col->getFieldOutputCycle() == 0){
            Eenergy = EMf->getEenergy(); Benergy = EMf->getBenergy(); TOTenergy = 0.0; TOTmomentum = 0.0;
            for (int is=0; is < ns; is++){
                Ke[is] = part[is].getKe(); TOTenergy += Ke[is];
                momentum[is] = part[is].getP(); TOTmomentum += momentum[is];
            }
            if(myrank == 0){
